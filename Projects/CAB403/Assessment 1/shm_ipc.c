...
bool do_work(shared_memory_t *shm)
{
    ...
    // as required.
    if (strcmp(op_names[shm->data->operation], "quit") == 0)
    {
        retVal = false;
    }
    else
    {
        if (strcmp(op_names[shm->data->operation], "add") == 0)
        {
            shm->data->result = shm->data->lhs + shm->data->rhs;
            printf("Add result: %d\n", shm->data->result);
        }
        else if (strcmp(op_names[shm->data->operation], "sub") == 0)
        {
            shm->data->result = shm->data->lhs - shm->data->rhs;
            printf("Subtract result: %d\n", shm->data->result);
        }
        else if (strcmp(op_names[shm->data->operation], "mul") == 0)
        {
            shm->data->result = shm->data->lhs * shm->data->rhs;
            printf("Multiply result: %d\n", shm->data->result);
        }
        else if (strcmp(op_names[shm->data->operation], "div") == 0)
        {
            {
                shm->data->result = shm->data->lhs / shm->data->rhs;
...
// C:\Users\roryb\Downloads\shm_ipc.c
