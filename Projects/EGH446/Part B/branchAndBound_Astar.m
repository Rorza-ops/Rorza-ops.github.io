function [Waypoints, Paths, Distance] = branchAndBound_Astar(map, waypoints, smoothing)
    
    %Total number of waypoints including start point
    N = length(waypoints);

    %Converting waypoints to world frame
    %waypoints = world2grid(map, waypoints);
    ...
    for i = 1:N
        for j = 2:N
            if i ~= j
                [path, info] = plan(planner, waypoints(i, :), waypoints(j, :), 'world');
                if smoothing
                    [newPath, ~, Distance] = pathSmoothing(map, {path});
                    paths{i, j} = newPath{1};
                    distanceMatrix(i, j) = Distance;
                else
                    paths{i, j} = path;
                    distanceMatrix(i, j) = info.PathCost;
                end
            end
        end
    end
    ...
    Paths = {};
    %paths{bestPath(1),bestPath(2)}
    
    for i = 1:N-1
        tempPath = paths{bestPath(i),bestPath(i+1)};
        Paths{i} = [Waypoints(i,:); tempPath(2:end-1,:); Waypoints(i+1,:)];
    end
